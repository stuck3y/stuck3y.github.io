...
- **Folder = app. Single `.html` file = reference sheet.** Anything in `/apps/<slug>/` is a real app: registered in `apps.json`, installable as a PWA (see below). Anything sitting in `/apps/` as a bare `.html` file is just a reference sheet — not registered, not a PWA, not advertised. If a reference sheet earns its place, fold it into `/apps/<slug>/index.html` and register it.
- **Exception: `iron.html` and `iron2.html`** are live production apps with thousands of users on stable bookmarked URLs. They predate this convention and stay as single-file apps registered in `apps.json` with `.html` paths. Do **not** folder-ify them, add PWA artifacts, or change their URLs without explicit approval — it will break user bookmarks.
...
- **Every folder app is a PWA.** That's the default, not an opt-in. New folder apps ship with these three lines in `<head>` and a `sw.js` next to `index.html`:
  ...
  Copy `/apps/lib/sw-app.js` to `./sw.js` and edit `CACHE` (e.g. `app-<slug>-v1`) and `SHELL` (the precache list — `['./', './index.html', './manifest.webmanifest']` plus any app-specific files). Add a `manifest.webmanifest` modeled on `/apps/ketchapp/manifest.webmanifest`; reference `../icon.svg` for the icon unless the app ships its own (`./icon.svg`). `/apps/manage/` is the one folder under `/apps/` that is *not* a registered app and not a PWA — it's the curation UI.
