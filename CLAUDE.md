...
- **The system SDK is `/apps/lib/sys.js` + `/apps/lib/tokens.css`.** Two files, zero dependencies, framework-agnostic. `sys.js` exposes `window.sys`: the app registry + `sys.launch(id)`, `sys.bus` (one origin-wide `BroadcastChannel` that also delivers locally to the sender), `sys.storage` (typed JSON over namespaced `localStorage`), and `sys.theme` (`set`/`init`, propagates live across open apps). `tokens.css` is the shared design language — SF stack, 4px rhythm, Apple blues, light/dark following the OS by default (`data-theme` to force), reduced-motion respected. Apps opt in by including both and calling `sys.theme.init()`. Shared **components** are deliberately absent until a pattern repeats three times — extract then, not before. **Full reference: [`/apps/SYSTEM.md`](./apps/SYSTEM.md)** — architecture, the SDK API surface, the bus event vocabulary, the river, the theme-signal model, and how to add/wire an app.
